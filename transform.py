...
CFG_PATH = "models/yolov4-obj.cfg"
WEIGHTS_PATH = "models/yolov4_2000.weights"
...
def perfromCropping_30(resImage,fileName,rois):
	...
	for roi,roi_im in zip(rois,im_arrays):
		os.makedirs(f"temp_files/{roi}/{name}")
		cv2.imwrite(f"temp_files/{roi}/{name}/{roi}.png",roi_im)
...
def makeFolders(rois):
	...
	for dir_path in rois:
		d_path = os.path.join("temp_files",dir_path)
...
def performDetectionRecognization(rois,questions):
	...
	try:
		for roi in rois:
			templatePath = Template(f"templates/{questions}/template_{roi}.json")
			roi_path = os.path.join("temp_files",roi)
		...
		response["message"] = "Please click the photo from the top of the sheet"
...
def main(im,imgPath,questions):
	
	drawn_im, coords = get_predictions(im.copy(), CFG_PATH, WEIGHTS_PATH, CLASSES_PATH)


	print("coords : ",coords)

	# w,h,_ = im.shape
	# drawn_im = cv2.resize(drawn_im,(h//2,w//2))
	# cv2.imshow("object detection", drawn_im)
	# cv2.imwrite("example.png",drawn_im)
	# cv2.waitKey(0)
...
if __name__ == "__main__":
	...
	questions = 20
	imgPath = "../testing/images2/23.jpg"
	
	im = cv2.imread(imgPath)
	print(main(im, imgPath, questions))
	
	"------------------------------------------------------"


	# questions = 20
	# ls = glob("../testing/images2/*")
	...
	# 			os.system(f"mv example.png ../../ppt_purpose/{i}.png")
	# 		else:
	# 			wrong += 1
